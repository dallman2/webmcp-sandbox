  ...
  ws.onmessage = async (event) => {
    ...
    if (msg.type === "call_tool") {
      ...
      try {
        ...
        ws?.send(JSON.stringify({ type: "tool_result", id: msg.id, result: result[0].result }));
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        ws?.send(JSON.stringify({ type: "tool_error", id: msg.id, error: message }));
      }
    }
    ...
    if (msg.type === "discover_tools") {
      ...
      ws?.send(JSON.stringify({ type: "discover_result", tools: result[0].result }));
    }
  };
}

async function callPageTool(toolName: string, args: Record<string, unknown>): Promise<unknown> {
  if (!document.modelContext) {
    throw new Error("document.modelContext not available — ensure @mcp-b/webmcp-polyfill is loaded");
  }
...
chrome.runtime.onInstalled.addListener(() => {
  console.log("[bridge] extension installed, connecting...");
  connect();
});
